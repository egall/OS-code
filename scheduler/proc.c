...
#include </usr/src/commands/swifi/random.c>
#include <time.h>
 ...
 */
  ...
  if (! time_left) {				/* quantum consumed ? */
      rp->p_ticks_left = rp->p_quantum_size; 	/* give new quantum */
                                                   /* egall */
      if ((rp->p_priority < (NR_SCHED_QUEUES-1) ) && (priv(rp)->s_flags & SYS_PROC)) {
    ...
    if( (priv(rp)->s_flags & SYS_PROC) ){
      ...
      return rp;
        ...
        for(rp = rdy_head[q]; rp != NIL_PROC; rp = rp->p_nextready){
          total_tickets += rp->lottery_tickets;
        }
        /* random_num = (random() % (total_tickets - 1) );  */
        for(rp = rdy_head[q]; rp != NIL_PROC; rp = rp->p_nextready){
          random_num -= rp->lottery_tickets;
          if(random_num <= 0){
            return rp;
        }
      } 
 ...
 */
  ...
  for (rp=BEG_PROC_ADDR; rp<END_PROC_ADDR; rp++) {
                            /*         egall              */
      if (! isemptyp(rp) && (priv(rp)->s_flags & SYS_PROC)) {				/* check slot use */
