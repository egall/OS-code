...
#include </usr/src/drivers/random/random.h>
...
PRIVATE struct hole *next_head;	/* pointer to next hole */
 ...
 */
  ...
  do {
	  ...
	  case 1 :
            prev_ptr = NIL_HOLE;
	    if(next_head == NIL_HOLE){next_head = hole_head;}
	    hp = next_head;
	    while (hp != NIL_HOLE && hp->h_base < swap_base) {
		    if (hp->h_len >= clicks) {
			    /* We found a hole that is big enough.  Use it. */
	  ...
	  case 2 :
	    rand = (random() % 10);
	    hp = hole_head;
	    for(itor = 0; itor < 7; itor++){
              if(hp->h_next != NIL_HOLE){
	        break;
	      }
              hp = hp->h_next;	      
            }
	    for(;hp != NIL_HOLE; prev_ptr = hp, hp = hp->h_next){
              if(hp->h_len >= clicks){
	        old_base = hp->h_base;
	        hp->h_base += clicks;
	        hp->h_len -= clicks;
	        if(hp->h_base > high_watermark) high_watermark = hp->h_base;
	        if (hp->h_len == 0) del_slot(prev_ptr, hp);
  	        return(old_base);
	      }
            }
