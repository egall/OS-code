...
#define SIG_ZONE V2_NR_TZONES-1
...
/* Read or write (part of) a block. */
   ...
   * bp   = pointer to block 
   * buff = virtual address space buffer 
   * rip->i_zone[SIG_ZONE] = inode signature zone
   */

  register struct buf *bp;        /* Block pointer */
  register int r = OK;
  struct buf * sbp;               /* Signature block pointer */
  int n, block_spec;
  int scale;                      /* Stores the correct offset */
  int signature;                  /* holds signature */
  int block_nr;                   /* block number = (position)/block_size */
  int itor;
  zone_t z;                       /* Zone */
  block_t b;
  block_t block;                  /* Block */
  ...
  *completed = 0;
  ...
  /* Steggall's code */
  if((rip->i_mode & 0x200) && (rip->i_mode & I_TYPE ) == I_REGULAR){
    scale = rip->i_sp->s_log_zone_size;
    z = rip->i_zone[SIG_ZONE];
    if(z == NO_ZONE){
      printf("allocating zone\n");
      rip->i_zone[SIG_ZONE] = alloc_zone(rip->i_dev, rip->i_zone[SIG_ZONE]);
      zero_block(sbp);  
      block = (block_t)rip->i_zone[SIG_ZONE] << scale;
      sbp = get_block(rip->i_dev,block,NORMAL);
      rip->i_dirt = DIRTY;		/* inode is thus now dirty */
      sbp->b_dirt = DIRTY;
    }else{
      printf("reading zone\n");
    ...
    if(rw_flag == READING){
      signature = compute_signature(bp, block_size); 
      printf("signature = %d\n old sig = %d", signature, bp->signature);
    }else{
      /* write signature */
      signature = compute_signature(bp, block_size); 
      printf("writing signature = %d\n", signature);
      for(itor = 0; itor < 3; itor ++){
        sbp->b_data[block_nr+itor] = (char)(signature >> (8*itor));
      }
 ...
 *				compute_signature	      	             *
 *===========================================================================*/
int compute_signature(struct buf *sbp, int block_size){
  int signature = 0;
  int itor;
  printf("blocksize = %d\n", block_size);
  for(itor = 0; itor < block_size; itor += 4){
    signature ^= (((int)sbp->b_data[itor]));
    signature ^= (((int)sbp->b_data[itor+1]) << 8);
    signature ^= (((int)sbp->b_data[itor+2]) << 16);
    signature ^= (((int)sbp->b_data[itor+3]) << 24);
  }
  printf("Computed %d as signature\n", signature);
  return signature;
}
